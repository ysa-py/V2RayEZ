...
| Phase 3 native CI/CD pipeline | `.github/workflows/vor-native-phase3.yml` + `tools/vor_phase3_pipeline_gate.mjs` + `scripts/build-openwrt-ipk.sh --no-fallback` | authored; CI execution blocked in this sandbox | `node tools/vor_phase3_pipeline_gate.mjs` PASS; YAML parse PASS; publisher count = 1 | Multi-runner workflow provisions Rust/Go/Android/JDK17/NDK26/Xcode/MSVC+WiX+cargo-xwin/webkit+eBPF/OpenWrt SDK, runs `cargo test --workspace --all-targets`, `go test ./...`, `./gradlew testDebugUnitTest`, then builds Android `.apk`+`.aab`, iOS `.ipa`, Tauri desktop bundles, OpenWrt `.ipk` (fail-closed `--no-fallback`), and uploads `SHA256SUMS.txt`. Real hashes must be captured after a runner execution. |
