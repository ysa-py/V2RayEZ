...
test('Windows UI uses the compact Android-parity navigation and Home layout', async () => {
  ...
  assert.doesNotMatch(html, /Upload Speed|Download Speed|How should V2RayEZ connect\?/);
...
test('License and AI Engine settings are additive, localized, and secret-free', async () => {
  const [html, app, i18n, settings, lib, license, ai, secure] = await Promise.all([
    read('../src/index.html'), read('../src/app.js'), read('../src/i18n.js'),
    read('../src-tauri/src/settings.rs'), read('../src-tauri/src/lib.rs'),
    read('../src-tauri/src/license.rs'), read('../src-tauri/src/ai_provider.rs'), read('../src-tauri/src/secure_store.rs')
  ]);
  for (const id of ['licenseSerial', 'licenseAccount', 'licenseValidationUrl', 'licenseOfflineGrace', 'activateLicense', 'validateLicense', 'aiProviderList', 'aiProviderId', 'aiProviderName', 'aiProviderType', 'aiBaseUrl', 'aiEndpoint', 'aiModel', 'aiApiAlias', 'aiSecret', 'aiHeaders', 'aiTemplate', 'aiResponsePath', 'saveAiProvider', 'testAiProvider']) assert.match(html, new RegExp(`id="${id}"`));
  for (const key of ['license.title', 'license.activate', 'ai.title', 'ai.providerId', 'ai.saveProvider', 'ai.secretHint']) for (const language of ['en', 'fa']) assert.ok(translations[language][key], `Missing ${language} translation: ${key}`);
  assert.match(settings, /LicenseSettings/);
  assert.match(settings, /AiEngineSettings/);
  assert.match(lib, /license::enforce\(app\.clone\(\), &settings\)\.await/);
  assert.match(lib, /spawn_license_watchdog/);
  assert.match(app, /activate_license/);
  assert.match(app, /test_ai_provider/);
  assert.match(license, /verify_license_key/);
  assert.match(ai, /auto_fallback_to_local|local_fallback/);
  assert.match(secure, /DPAPI|0o600/);
  assert.doesNotMatch(settings, /apiSecret|license_key/);
});
...
test('About, Telegram, and opener permissions are complete', async () => {
  ...
  assert.match(html, /ysa-py\/V2RayEZ/);
  assert.match(app, /tg:\/\/resolve\?domain=v2rayez/);
  assert.match(app, /https:\/\/t\.me\/v2rayez/);
  assert.doesNotMatch(html, /(?:src|href)="https?:/);
  const opener = JSON.parse(capability).permissions.find(item => item.identifier === 'opener:allow-open-url');
  for (const url of ['tg://resolve?domain=v2rayez', 'https://t.me/v2rayez', 'https://github.com/CluvexStudio/Aether', 'https://github.com/ysa-py/V2RayEZ']) assert.ok(opener.allow.some(item => item.url === url));
});
...
test('application metadata and visible release version are 2.0.0', async () => {
  ...
  assert.equal(JSON.parse(pkg).name, '@v2rayez/universal-gui');
  assert.equal(JSON.parse(pkg).version, '2.0.0');
  const tauriConfig = JSON.parse(tauri);
  assert.equal(tauriConfig.productName, 'V2RayEZ');
  assert.equal(tauriConfig.identifier, 'app.v2rayez.universal');
  assert.equal(tauriConfig.version, '2.0.0');
  for (const target of ['nsis', 'msi', 'deb', 'rpm', 'appimage']) assert.ok(tauriConfig.bundle.targets.includes(target), `missing bundle target ${target}`);
  assert.match(cargo, /name = "v2rayez-gui"/);
  assert.match(cargo, /version = "2\.0\.0"/);
  assert.doesNotMatch(html + app + tauri + cargo, /Ae(?:thon|therGUI)/);
...
test('Android reference remains v2.0.0 with VPNService, RTL, and application picker', async () => {
  const [manifest, activity, service, picker, gradle] = await Promise.all([
    ...
    read('../android/app/src/main/java/app/v2rayez/gui/MainActivity.java'),
    read('../android/app/src/main/java/app/v2rayez/gui/AetherVpnService.java'),
    read('../android/app/src/main/java/app/v2rayez/gui/AppSelectionActivity.java'),
