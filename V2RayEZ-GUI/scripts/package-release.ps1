...
Copy-Item -LiteralPath (Join-Path $repo "src-tauri/target/release/bundle/nsis/V2RayEZ_${Version}_x64-setup.exe") -Destination (Join-Path $releaseDir "V2RayEZ-VPN-v${Version}-Windows-x64-Installer.exe")
Copy-Item -LiteralPath (Join-Path $repo "src-tauri/target/release/bundle/msi/V2RayEZ_${Version}_x64_en-US.msi") -Destination (Join-Path $releaseDir "V2RayEZ-VPN-v${Version}-Windows-x64.msi")
Copy-Item -LiteralPath (Join-Path $repo "src-tauri/target/release/v2rayez-gui.exe") -Destination (Join-Path $portableDir "V2RayEZ.exe")
...
Compress-Archive -Path (Join-Path $portableDir "*") -DestinationPath (Join-Path $releaseDir "V2RayEZ-VPN-v${Version}-Windows-x64-portable.zip") -Force

$androidOutputs = @{
    "android/app/build/outputs/apk/release/app-universal-release.apk" = "V2RayEZ-VPN-v${AndroidVersion}-Android-Universal.apk"
    "android/app/build/outputs/apk/release/app-armeabi-v7a-release.apk" = "V2RayEZ-VPN-v${AndroidVersion}-Android-ARMv7.apk"
    "android/app/build/outputs/apk/release/app-arm64-v8a-release.apk" = "V2RayEZ-VPN-v${AndroidVersion}-Android-ARM64.apk"
    "android/app/build/outputs/apk/release/app-x86_64-release.apk" = "V2RayEZ-VPN-v${AndroidVersion}-Android-x86_64.apk"
    "android/app/build/outputs/bundle/release/app-release.aab" = "V2RayEZ-VPN-v${AndroidVersion}-Android-AAB.aab"
}
...
$bundle = Join-Path $releaseDir "V2RayEZ-VPN-v${Version}-all-platforms.zip"
