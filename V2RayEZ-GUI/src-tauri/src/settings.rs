...
impl Default for LicenseSettings {
    fn default() -> Self {
        Self {
            ...
            allow_offline_grace: true,
            last_result: "not_validated".into(),
...
pub struct AiEngineSettings {
    ...
    pub last_provider_test_result: String,
}
...
pub struct Settings {
    ...
    pub ai_engine: AiEngineSettings,
}
...
impl Default for Settings {
    fn default() -> Self {
        Self {
            ...
            ai_engine: AiEngineSettings::default(),
        }
    }
}
...
impl Settings {
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        validate_urlish("license validation URL", &self.license.validation_url, false)?;
        if self.license.account_id.contains(['\0', '\n', '\r']) {
            return Err("Invalid license account ID".into());
        }
        for provider in &self.ai_engine.providers {
            if provider.id.trim().is_empty() || provider.id.contains(['\0', '/', '\\']) {
                return Err("AI provider id is invalid".into());
            }
            one_of(
                "AI provider type",
                &provider.provider_type,
                &["openai", "anthropic", "gemini", "generic", "local"],
            )?;
            if provider.provider_type != "local" {
                validate_urlish("AI provider base URL", &provider.base_url, true)?;
            }
            if !(2_000..=120_000).contains(&provider.timeout_ms) {
                return Err("AI provider timeout must be between 2000 and 120000 ms".into());
            }
            serde_json::from_str::<serde_json::Value>(&provider.headers_json)
                .map_err(|_| "AI provider headers must be valid JSON".to_string())?;
        }
        Ok(())
    }
...
fn validate_urlish(label: &str, value: &str, required: bool) -> Result<(), String> {
    let value = value.trim();
    if value.is_empty() {
        return if required { Err(format!("{label} is required")) } else { Ok(()) };
    }
    if value.contains(['\0', ' ', '\n', '\r']) {
        return Err(format!("{label} contains invalid characters"));
    }
    if value.starts_with("https://") || value.starts_with("http://") || value.starts_with("local://") {
        Ok(())
    } else {
        Err(format!("{label} must start with https://, http://, or local://"))
    }
}
