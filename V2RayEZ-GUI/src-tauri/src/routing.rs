        ...
        if let Some(dir) = existing_dir {
            match read_status(&dir).map(|status| status.state) {
                Ok(state) if !matches!(state.as_str(), "error" | "disabled") => {
                    return Err("A V2RayEZ routing session is already active".into());
                }
                _ => {
...
impl RoutingRequest {
    fn validate(&self) -> Result<(), String> {
        ...
        if self.tun_interface.len() > 32
            ...
            || !self.tun_interface.starts_with("V2RayEZTun-")
        {
            return Err("Invalid V2RayEZ TUN interface name".into());
        }
...
fn sing_box_config(request: &RoutingRequest) -> Value {
    ...
    let mut rules = vec![
        ...
        json!({"process_name":["aether.exe","v2rayez-gui.exe","V2RayEZ.exe","sing-box.exe"],"action":"route","outbound":"direct"}),
    ];
...
pub fn helper_main(request_path: &Path) -> Result<(), String> {
    ...
    let result = (|| -> Result<(), String> {
        ...
        write_status(
            &request.session_dir,
            "starting-adapter",
            "Starting the V2RayEZ virtual adapter",
            0,
        )?;
...
fn wait_for_tun_ready(
    ...
    loop {
        ...
        if std::time::Instant::now() >= deadline {
            ...
            let _ = child.wait();
            return Err(format!(
                "Routing engine did not create a ready V2RayEZ TUN adapter.{}",
                read_log_tail(log_path)
            ));
        }
...
/// bypass the V2RayEZ /0 route even though the TUN adapter is ready. Install
/// active-session /1 routes on our adapter so system traffic follows V2RayEZ.
fn install_takeover_routes(interface_name: &str) -> Result<(), String> {
    #[cfg(windows)]
    {
        for prefix in ["0.0.0.0/1", "128.0.0.0/1"] {
            ...
            if !output.status.success() {
                return Err(format!(
                    "Could not install the V2RayEZ {prefix} route: {}",
        ...
        if !metric_output.status.success() {
            return Err(format!(
                "Could not prioritize the V2RayEZ TUN interface: {}",
...
fn acquire_helper_lock() -> Result<HelperLock, String> {
    #[cfg(windows)]
    {
        ...
        let name = wide("Local\\V2RayEZRoutingHelper");
        for _ in 0..20 {
            let handle = unsafe { CreateMutexW(std::ptr::null(), 0, name.as_ptr()) };
            if handle.is_null() {
                return Err("Could not create the V2RayEZ routing-session lock".into());
            }
        ...
        Err("Another V2RayEZ routing helper is still active".into())
    }
    #[cfg(not(windows))]
    {
        Ok(HelperLock)
...
fn recover_owned_session(session_dir: &Path) {
    ...
    let _ = write_status(
        session_dir,
        "disabled",
        "Previous V2RayEZ routing session was recovered",
        0,
    );
}
...
fn cli_recovery_path() -> Option<PathBuf> {
    std::env::var_os("LOCALAPPDATA")
        .map(|v| PathBuf::from(v).join("V2RayEZ-routing-recovery.json"))
}
...
/// Remove only adapters V2RayEZ creates. The explicit legacy name is included
...
fn cleanup_owned_adapters(keep: Option<&str>) {
    #[cfg(windows)]
    {
        ...
        let script = format!(
            "$keep='{keep}'; Get-PnpDevice -Class Net -ErrorAction SilentlyContinue | Where-Object {{ ($_.FriendlyName -eq 'V2RayEZLegacyTun' -or $_.FriendlyName -like 'V2RayEZTun-*') -and $_.FriendlyName -ne $keep }} | ForEach-Object {{ & pnputil.exe /remove-device $_.InstanceId | Out-Null }}"
        );
        let mut command = Command::new("powershell.exe");
        command.args([
            "-NoProfile",
...
mod tests {
    ...
    fn request() -> RoutingRequest {
        RoutingRequest {
            ...
            tun_interface: "V2RayEZTun-1-2".into(),
    ...
    fn full_config_uses_socks_and_strict_routes() {
        ...
        assert!(c.contains("V2RayEZTun-1-2"));
    }
    ...
    fn atomic_json_replaces_existing_status() {
        let dir = std::env::temp_dir().join(format!("v2rayez-routing-test-{}", std::process::id()));
