        ...
        loop {
            ...
            match license::enforce(app.clone(), &settings).await {
                ...
                Err(error) => {
                    ...
                    emit_status(&app, "error", None, Some(error));
                    return;
                }
            }
        }
...
async fn start_core_with_fallback(
    ...
    if let Some(advice) = ai_provider::advise_on_failure(app, settings, &primary_error).await {
        let _ = app.emit(
            "aether-log",
            format!("AI Engine advisor ({}): {}", advice.source, advice.text),
        );
    }
...
fn tray_menu(app: &AppHandle) -> tauri::Result<Menu<tauri::Wry>> {
    let show = MenuItem::with_id(app, "show", "Show V2RayEZ", true, None::<&str>)?;
...
async fn activate_license(
    ...
    license::activate(app, settings, license_key).await
}
...
async fn validate_license(
    ...
    license::validate(app, settings, false).await
}

#[tauri::command]
async fn clear_license(app: AppHandle) -> Result<license::LicenseStatus, String> {
    license::clear(app)
}
...
) -> Result<ai_provider::AiProviderResult, String> {
    ai_provider::test_provider(app, settings, provider_id, api_secret).await
}

#[tauri::command]
async fn save_ai_secret(app: AppHandle, alias: String, secret: String) -> Result<(), String> {
    ai_provider::save_secret(app, alias, secret)
}
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .expect("error while running V2RayEZ");
}
