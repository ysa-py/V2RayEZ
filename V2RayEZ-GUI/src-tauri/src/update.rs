...
const RELEASE_API: &str = "https://api.github.com/repos/ysa-py/V2RayEZ/releases?per_page=30";
const DOWNLOAD_PREFIX: &str = "https://github.com/ysa-py/V2RayEZ/releases/download/";
...
fn client() -> Result<reqwest::Client, String> {
    reqwest::Client::builder()
        .user_agent("V2RayEZ-Update/2.0.0")
...
async fn checksum_from_manifest(
    ...
    if !asset.browser_download_url.starts_with(DOWNLOAD_PREFIX) {
        return Err("The checksum URL is not an official V2RayEZ release URL".into());
    }
    let text = http
...
async fn latest_update() -> Result<UpdateInfo, String> {
    ...
    let (release, latest, installer_name, asset) = releases
        ...
        .filter_map(|release| {
            ...
            let installer_name = format!("V2RayEZ-VPN-v{latest}-Windows-x64-Installer.exe");
    ...
    if !asset.browser_download_url.starts_with(DOWNLOAD_PREFIX) {
        return Err("The installer URL is not an official V2RayEZ release URL".into());
    }
...
pub async fn download_update(
    ...
    let info = latest_update().await?;
    if !info.available {
        return Err("V2RayEZ is already up to date".into());
    }
    let directory = app
        .path()
        .app_cache_dir()
    ...
    tokio::fs::create_dir_all(&directory)
        .await
        .map_err(|error| error.to_string())?;
    let filename = format!("V2RayEZ_{}_x64-setup.exe", info.latest_version);
