...
| Android donor anti-fabrication (fail-closed) | `MICAFP/android/app/src/main/kotlin/com/unifiedshield/{micafp/{EbpfSocketFilterEngine,MicafpKernelRingBufferEngine,DiagnosticTelemetryService,MicafpQuantumMorphProtocol,OnDeviceNeuralReconEngine,TfLitePacketAnalyzerEngine}.kt,AiStealthEngine.kt,DpiDiagnosticEngine.kt,aiorchestrator/{AdaptiveNetworkProfiler,AiCoreOrchestrator,DpiTfLiteAnomalyDetector}.kt,resilience/NetworkClientManager.kt,tunnel/DualModeTransportEngine.kt,ui/{MicafpQuantumDashboardPanel,AiEngineScreen,StatusCard,ThreatIntelPanel,DpiDiagnosticScreen,DiagnosticTelemetryCharts,AdvancedToolsScreen,DualModeTransportScreen}.kt}`, `tools/vor_anti_fabrication_gate.mjs` | Android donor | merged (fail-closed cleanup) | `node tools/vor_anti_fabrication_gate.mjs` PASS. 13 Android donor engines now fail closed with `backendUnavailable`/zero telemetry instead of `Random.next*`; real backoff jitter retained; UI renders honest unavailable states. |
| Android donor anti-fabrication follow-up (previously-inventoried donors) | `MICAFP/android/app/src/main/kotlin/com/unifiedshield/{cottendns/{CottenDnsEngine,CottenDnsModels}.kt,stormdns/{StormDnsEngine,StormDnsModels}.kt,tunnel/{MasterDnsEngine,MasterDnsConfig,TunnelProfile}.kt,whitedns/{WhiteDnsModels,WhiteDnsScannerEngine}.kt,scanner/AutoScannerEngine.kt,profile/ProfileManager.kt,UnifiedShieldStore.kt,security/SecurityController.kt,logging/DebugLogger.kt,ui/{TunnelsScreen,CottenDnsScreen,ThreatIntelPanel,MasterDnsScreen,StormDnsScreen,WhiteDnsScreen,Quantum3DParticleCanvas}.kt}`, `tools/vor_anti_fabrication_gate.mjs` | Android donor | merged (fail-closed cleanup) | `node tools/vor_anti_fabrication_gate.mjs` PASS. CottenDNS/StormDNS/MasterDNS/WhiteDNS/AutoScanner/ProfileManager/UnifiedShieldStore/SecurityController/DebugLogger are now fail-closed with zero/unavailable telemetry and honest UI; DNS engine real-data sample entry points refuse unmeasured samples. AutoScanner `startFullAutoScan` returns empty results with `backendUnavailable=true`; `.random()` remains only as labeled candidate-target enumeration (`REAL DISCOVERY ONLY`). ProfileManager default `pingMs=0`, `measured=false`, and `autoApplyFromScanner` refuses unmeasured nodes. UnifiedShieldStore no longer feeds fake `latencyMs=16` to `AiStealthEngine`. Quantum3DParticleCanvas is labeled `VISUALIZATION ONLY — NOT REAL TELEMETRY`. |
| Rust offline serial (source refactor) | `universal-core/src/license.rs`, `universal-core/src/bin/v2rayez-license-gate.rs`, `V2RayEZ-GUI/src-tauri/src/license.rs`, `tools/vor_rust_license_static_gate.mjs` | All native shells | source refactored; **cargo-test blocked** | `node tools/vor_rust_license_static_gate.mjs` PASS; `node tools/license_serial_e2e_selftest.mjs` PASS; `cargo test` → `cargo: command not found` (exit 127). `offline_start_decision` now verifies serial-level `device_id_hash`, verifies/schema-checks a signed `V2RayEZ-Revocation-List`, and allows a valid signed serial with NO grace-token fallback. Must be type-checked/unit-tested on Rust-capable runner before `merged`. |
| Phase 2 native release binaries | Android `.apk`+`.aab`, iOS `.ipa`+PacketTunnel, Windows `.exe`, Linux/macOS ELF/`.dmg`, OpenWrt `.ipk` | Android, iOS, Windows, Linux/macOS, OpenWrt | **blocked locally** | `bash scripts/build-release-artifacts.sh` exits 1: `error: required tool 'java' is not installed for target 'android'`; `cargo`/`go`/`gradle`/`xcodebuild`/`msbuild`/`cl` all `command not found`; `static.crates.io`/`proxy.golang.org`/`dl.google.com`/`maven.google.com` unreachable. Zero Fabrication rule honored: no placeholder/stub artifacts emitted; checksums must be appended on a toolchain-equipped runner. |
| Phase 3 native CI/CD pipeline | `.github/workflows/vor-native-phase3.yml`, `tools/vor_phase3_pipeline_gate.mjs`, `scripts/build-openwrt-ipk.sh`, `V2RayEZ-GUI/scripts/fetch-sidecars.mjs`, `MICAFP/go-bridge` v0.5 port, `EasySNI` pinned optional deps, `MasterDnsVPN` deterministic near-top round-robin | Android, iOS, Windows, Linux/macOS, OpenWrt | native-tests PASS on runner; tag-triggered full matrix runs but is **signed-artifact-blocked by unavailable signing secrets**; desktop/OpenWrt blockers reduced to code-level issues fixed below, pending re-validation after the user cancelled the last run | Java/No-run CI: `33926134449` (commit `19d7183`) = native-tests ✓ + zero tar/cache annotations; `33925274633` = zero warnings. Tag runs `33926430878`–`33943171341`: cache collision fixed; Android JNI + `assembleRelease`/`bundleRelease` succeed, signing fail-closes when `VOR_ANDROID_*` absent; iOS fail-closes on missing Apple secrets; desktop sidecars now provisioned by `fetch-sidecars.mjs` (Aether verified against upstream SHA256SUMS.txt, sing-box digest computed), builds moved to `npx tauri build`, Tauri RGBA icon requirement fixed (all icons color type 6), windows-sys 0.59 DPAPI API fixed (`CRYPT_INTEGER_BLOB` + manual `LocalFree`); OpenWrt cross-compile PASS, package target moved to `package/unifiedshield` so `make package/unifiedshield/compile` resolves, `--no-fallback` still enforced; diagnostics ANSI-strip + artifact/annotation capture added. No fabricated hashes recorded; `SHA256SUMS.txt` must follow a successful `checksum-ledger` run. Journey: removed corrupt `MICAFP/go-bridge/go.mod` indirect block, added fail-closed `go mod tidy` + exact diagnostics as annotations; ported `MICAFP/go-bridge` to `yggdrasil-go v0.5.14` (`src/tun`/`tun.TunAdapter`, `core.New`, `ipv6rwc`, `WriteTo`, URL peers) and fixed cgo prototypes + mobile StateCallback trampoline; `MICAFP/go-bridge` + `yggdrasil-mobile` pass. Donor modules are no longer skipped: `EasySNI` optional LiveKit/Psiphon manifests are pinned so `-tags livekit,psiphon` builds are supported, and `MasterDnsVPN` near-top loss-then-latency selection is deterministic and testable. `node tools/vor_phase3_pipeline_gate.mjs` PASS; YAML parses; publisher count = 1. |
