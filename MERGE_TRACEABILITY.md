...
| Phase 3 native CI/CD pipeline | `.github/workflows/vor-native-phase3.yml`, `tools/vor_phase3_pipeline_gate.mjs`, `scripts/build-openwrt-ipk.sh`, `MICAFP/go-bridge` v0.5 port, `EasySNI` pinned optional deps, `MasterDnsVPN` deterministic near-top round-robin | Android, iOS, Windows, Linux/macOS, OpenWrt | native-tests PASS on runner; tag-triggered full matrix runs but is **signed-artifact-blocked by unavailable signing secrets** and desktop/OpenWrt build blockers below | Push run `33926134449` (commit `19d7183`): cargo test, all Go modules, Android `testDebugUnitTest` all ✓ with zero tar/cache annotations after `cache: false` + `v4-cache-*` key bump. Push run `33925274633`: zero warnings. Authorized tag `vor-v1.0.0` triggered real full matrix runs `33926430878`/`33927089757`/`33928606078`/`33929199328`/`33929715629`. Findings: Android JNI + `assembleRelease`/`bundleRelease` now succeed (CI debug keystore generated, release keystore materialized from secrets when present); `signed=true` on tag makes iOS/Android signing fail-closed when secrets are absent; desktop build moved from `cargo tauri` (not installed) to `npx tauri build`, remaining blocker is real per-target Aether/sing-box sidecar provisioning; OpenWrt cross-compile still fails with captured `ph3-diag-openwrt-*` diagnostics. No fabricated hashes recorded; `SHA256SUMS.txt` must follow a successful `checksum-ledger` run. Journey: removed corrupt `MICAFP/go-bridge/go.mod` indirect block, added fail-closed `go mod tidy` + exact diagnostics as annotations; ported `MICAFP/go-bridge` to `yggdrasil-go v0.5.14` (`src/tun`/`tun.TunAdapter`, `core.New`, `ipv6rwc`, `WriteTo`, URL peers) and fixed cgo prototypes + mobile StateCallback trampoline; `MICAFP/go-bridge` + `yggdrasil-mobile` pass. Donor modules are no longer skipped: `EasySNI` optional LiveKit/Psiphon manifests are pinned so `-tags livekit,psiphon` builds are supported, and `MasterDnsVPN` near-top loss-then-latency selection is deterministic and testable. `node tools/vor_phase3_pipeline_gate.mjs` PASS; YAML parses; publisher count = 1. |
