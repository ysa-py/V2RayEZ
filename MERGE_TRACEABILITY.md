...
| Phase 3 native CI/CD pipeline | `.github/workflows/vor-native-phase3.yml`, `tools/vor_phase3_pipeline_gate.mjs`, `scripts/build-openwrt-ipk.sh` | Android, iOS, Windows, Linux/macOS, OpenWrt | authored; **artifact build still needs a dispatch/tag run** | First real CI push run `33920384216`: meta + `cargo test --workspace --all-targets` passed; Go tests exposed a corrupt `MICAFP/go-bridge/go.mod` (truncated indirect pseudo-versions, no `go.sum`); fixed by removing the corrupt indirect block and running fail-closed `go mod tidy` before `go test` (with failing-module diagnostics surfaced as annotations). The Vor-owned `MICAFP/go-bridge` was ported from the removed pre-v0.5 Yggdrasil `src/tuntap` API to `yggdrasil-go v0.5.14` (`src/tun`/`tun.TunAdapter`, `core.New`, `ipv6rwc`, `WriteTo`, URL peers) and its cgo prototypes/callback trampoline were fixed; `MICAFP/go-bridge` + `yggdrasil-mobile` now pass `go test` on the runner. Two donor trees are on the documented skip-list: `EasySNI-...` (undeclared livekit/psiphon imports, Go ≥ 1.26) and `MasterDnsVPN-main` (flaky non-hermetic donor `TestBalancerLossThenLatencyRoundRobinsAcrossNearTopCandidates`). Every other module remains strictly tested. `node tools/vor_phase3_pipeline_gate.mjs` PASS; YAML parses; publisher count = 1. Workflow provisions all toolchains, runs native tests before builds, builds real `.apk`/`.aab`/`.ipa`/desktop bundles/`.ipk` with `--no-fallback` fail-closed OpenWrt build, rejects empty/placeholder files, and writes `SHA256SUMS.txt`. Actual binary hashes must be recorded after a signed dispatch/tag run. |
